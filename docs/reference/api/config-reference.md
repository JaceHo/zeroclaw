...
| `max_tool_iterations` | `25` | Maximum tool-call loop turns per user message across CLI, gateway, and channels |
| `max_history_messages` | `100` | Maximum conversation history messages retained per session |
| `parallel_tools` | `true` | Enable parallel tool execution within a single iteration |
| `tool_dispatcher` | `auto` | Tool dispatch strategy |

Notes:

- Setting `max_tool_iterations = 0` falls back to safe default `25`.
