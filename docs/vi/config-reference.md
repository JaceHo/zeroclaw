...
| `max_tool_iterations` | `25` | Số vòng lặp tool-call tối đa mỗi tin nhắn trên CLI, gateway và channels |
| `max_history_messages` | `100` | Số tin nhắn lịch sử tối đa giữ lại mỗi phiên |
| `parallel_tools` | `true` | Bật thực thi tool song song trong một lượt |
| `tool_dispatcher` | `auto` | Chiến lược dispatch tool |

Lưu ý:

- Đặt `max_tool_iterations = 0` sẽ dùng giá trị mặc định an toàn `25`.
